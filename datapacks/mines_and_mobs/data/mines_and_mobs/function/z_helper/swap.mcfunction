...
$execute if data storage mines_and_mobs:temp swap run setblock ~ ~-1 ~ minecraft:chain_command_block[facing=$(direction)]{auto:1b} replace

execute if data storage mines_and_mobs:temp swap run return 0

clone ~1 ~-1 ~ ~1 ~-1 ~ ~ ~-1 ~ replace move
$setblock ~1 ~-1 ~ minecraft:chain_command_block[facing=$(direction)]{auto:1b} replace