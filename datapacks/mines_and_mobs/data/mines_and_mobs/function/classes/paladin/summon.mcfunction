data modify storage mines_and_mobs:class class merge value {name: "paladin", display_name: "Paladin"}
data modify storage mines_and_mobs:class class merge value {dyed_color: 16701501}
data modify storage mines_and_mobs:class class merge value {hex_color: "#FFFF00"}
data modify storage mines_and_mobs:class class merge value {weapon: "minecraft:golden_sword"}

function mines_and_mobs:classes/generic/summon with storage mines_and_mobs:class class